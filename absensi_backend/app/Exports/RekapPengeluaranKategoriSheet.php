...
class RekapPengeluaranKategoriSheet implements FromCollection, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'REKAPITULASI PENGELUARAN BERDASARKAN KATEGORI');
                ...
                // Group by category
                $grouped = $this->pengeluaran->groupBy(fn ($item) => $item->kategori ?: 'Lain-lain');
                $grandTotal = (float) $this->pengeluaran->sum('jumlah');
                ...
                foreach ($grouped as $kategori => $items) {
                    ...
                    $sheet->setCellValue("E{$rowNum}", "=D{$rowNum}/D" . (count($grouped) + 5));
