...
class DataSiswaScreen extends StatefulWidget {
  ...
  const DataSiswaScreen({super.key, this.readOnly = false});
...
class _DataSiswaScreenState extends State<DataSiswaScreen>
  ...
  void _selectAllFilteredSiswa() {
    if (widget.readOnly) return;
    final ids = _filteredSiswa.map(_siswaId).where((id) => id > 0).toSet();
    if (ids.isEmpty) {
  ...
  Future<void> _handleTambahSiswa() async {
    if (widget.readOnly) {
      _showSnack('Guru hanya dapat melihat data siswa.', isError: true);
      return;
    }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    widget.readOnly
                        ? 'Buku Induk - Data Siswa/Santri (lihat saja)'
        ...
        child: Column(
          children: [
            InkWell(
              borderRadius: BorderRadius.circular(20),
              onTap: () {
                if (!widget.readOnly && _isSelectionMode) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: widget.readOnly || _isSelectionMode
          ? null
